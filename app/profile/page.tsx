			...
			<div className="max-w-2xl mx-auto">
				...
				<div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 mb-6">
					<div className="flex items-center justify-between mb-4">
						...
						<div className={`px-3 py-1 rounded-full text-sm font-semibold ${profile?.is_premium ? 'bg-green-500/20 text-green-400 border border-green-700' : 'bg-gray-800 text-gray-400 border border-gray-700'}`}>
				...
				<div className="flex gap-2 mb-6">
					<button onClick={() => setActiveTab('stats')} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${activeTab === 'stats' ? 'bg-green-500 text-black' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
						My Stats
					</button>
					<button onClick={() => setActiveTab('badges')} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${activeTab === 'badges' ? 'bg-green-500 text-black' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
						...
						<div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
							...
							{results.length === 0 ? (
								...
								<>
									...
									{results.length > 5 && (
										<button onClick={() => setShowAllHistory(!showAllHistory)} className="w-full mt-4 py-2 text-sm text-green-400 hover:text-green-300 transition">
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-1 gap-3">
							{allBadgeTypes.map((badge) => {
								...
								return (
									<div key={badge.id} className={`flex items-center gap-4 p-4 rounded-xl border ${earned ? 'bg-gray-900 border-gray-700' : 'bg-gray-900/50 border-gray-800 opacity-50'}`}>
										<div className="text-3xl">{badge.emoji}</div>
										<div className="flex-1">
											<div className={`font-semibold ${earned ? 'text-white' : 'text-gray-500'}`}>{badge.name}</div>
